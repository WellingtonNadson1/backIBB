...
const routerRelatorioPresencaCelula = async (fastify: FastifyInstance) => {
  fastify.post("/relatorio/presencacelula", async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const membrosCompareceramReuniaoCelula = await prisma?.user.findMany({
        where: {
          ...
          presencas_reuniao_celula: {
            where: {
              which_reuniao_celula: {
                AND: [
                  { data_reuniao: { gte: dataInicio } },
                  { data_reuniao: { lte: dataFim } }
                ]
              }
            },
            select: {
              // Selecione apenas os campos relevantes em 'presencas_reuniao_celula'
              status: true,
              id: true,
              date_create: true,
              which_reuniao_celula: {
      ...
      const membrosCompareceramReuniaoCelulaFiltrados = membrosCompareceramReuniaoCelula.map((membro) => {
        const presencasFiltradas = membro.presencas_reuniao_celula.filter((presenca) => {
        ...
        //   return total + ((presente.status === true && presente.?.culto_semana?.id === 'bffb62af-8d03-473a-ba20-ab5a9d7dafbe') ? 1 : 0);
        // }, 0);

        // const quantidadeCultosPresenteDomingoSacrificio = presencasFiltradas.reduce((total, presente) => {
        //   return total + ((presente.status === true && presente.?.culto_semana?.id === 'e7bc72d1-8faa-4bbe-9c24-475b64f956cf') ? 1 : 0);
        // }, 0);

        // const quantidadeCultosPresenteQuarta = presencasFiltradas.reduce((total, presente) => {
        //   return total + ((presente.status === true && presente.?.culto_semana?.id === '4064be1d-bf55-4851-9f76-99c4554a6265') ? 1 : 0);
        // }, 0);

        // const quantidadeCultosPresenteSabado = presencasFiltradas.reduce((total, presente) => {
        //   return total + ((presente.status === true && presente.?.culto_semana?.id === '84acfbe4-c7e0-4841-813c-04731ffa9c67') ? 1 : 0);
        // }, 0);

        // const quantidadeCultosPresenteDomingoManha = presencasFiltradas.reduce((total, presente) => {
        //   return total + ((presente.status === true && presente.?.culto_semana?.id === 'cab02f30-cade-46ca-b118-930461013d53') ? 1 : 0);
        // }, 0);

        // const quantidadeCultosPresenteDomingoTarde = presencasFiltradas.reduce((total, presente) => {
        //   return total + ((presente.status === true && presente.?.culto_semana?.id === 'ea08ec9b-3d1b-42f3-818a-ec53ef99b78f') ? 1 : 0);
        // }, 0);
      ...
      reply.send(membrosCompareceramReuniaoCelulaFiltrados);
    } catch (error) {
